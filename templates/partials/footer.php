</main>

    <footer class="bg-verde-escuro py-4 mt-7 text-center text-white">
        
        <div class="container max-w-[1200px] mx-auto px-5">
            
            <p class="small mb-2 opacity-80">
                Todo o processo de estágio segue as normativas legais vigentes e os regulamentos internos do IFSP Campus Guarulhos.
            </p>
            
            &copy; <?php echo date('Y'); ?> IFSP - Campus Guarulhos • Coordenação de Estágios.  
            <br>
            
            <div class="mt-2 text-sm space-x-2">
                <a href="politica-privacidade" class="text-verde-claro hover:underline">Política de Privacidade</a> 
                <span>|</span>
                <a href="acessibilidade" class="text-verde-claro hover:underline">Acessibilidade</a>
            </div>
            
        </div>
    </footer>
<script src="https://cdn.tailwindcss.com"></script>
<script src="<?= htmlspecialchars($BASE_URL . 'assets/js/script.js', ENT_QUOTES, 'UTF-8') ?>"></script>
<script src="<?= htmlspecialchars($BASE_URL . 'assets/js/form-scripts.js', ENT_QUOTES, 'UTF-8') ?>"></script>


</body>
</html>